import { AppBar, Card, CardContent, CardActions, Button, Divider, Grid, InputAdornment, IconButton, Paper, Select, Table, MenuItem, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Toolbar, Typography, CardHeader } from '@material-ui/core';
import { makeStyles } from '@material-ui/core/styles';
import React, { useEffect, useState } from 'react';
import { FaCalculator,FaPlayCircle, FaEquals, FaGreaterThanEqual, FaLessThanEqual, FaPlus } from 'react-icons/fa';
...
function App() {
  ...
  const calculate = () => {
    ...
    setSolution(oldSolution => [...oldSolution,
    <Typography align='center' variant='h3'>Solution</Typography>
    ])
    ...
    while (true) {
      ...
      if (result === 0) {
        ...
        <div>
          ...
          <Typography align='center' variant='h3'>End of Solution</Typography>
        </div>
        ]);
        break;
      } else {
        ...
        let z = math.multiply(basic.map(b => c_all[b]), xb);
              ...
              <Table className={classes.table} aria-label="simple table">
                <TableHead>
                  <TableRow>
                    ...
                    {basicPrint.map(b => <TableCell>x{b + 1}</TableCell>)}
                  </TableRow>
                  <TableRow>
                    <TableCell>Non-Basic Variables</TableCell>
                    {nonBasicPrint.map(b => <TableCell>x{b + 1}</TableCell>)}
                  </TableRow>
                  <TableRow>
      ...
      <TableContainer component={Paper}>
        <Table className={classes.table} aria-label="simple table">
          ...
          <TableBody>
            ...
            <TableRow>
              <TableCell align='center' colSpan={(varCount * 2)}>
                <Button
                  variant="contained"
                  color="primary"
                  onClick={() => calculate()}
